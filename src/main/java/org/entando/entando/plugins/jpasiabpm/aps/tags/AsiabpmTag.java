...
public class AsiabpmTag extends TagSupport {

	private static final Logger _logger =  LoggerFactory.getLogger(AsiabpmTag.class);
	...
	public int doStartTag() throws JspException {
		...
		Asiabpm asiabpm = null;
			if (null != this.getKey()) {
				asiabpm = asiabpmManager.getAsiabpm(this.getKey());
			} else {
				Widget widget = this.extractWidget(reqCtx);
				ApsProperties widgetConfig = widget.getConfig();
				String varid = widgetConfig.getProperty("id");
				if (StringUtils.isNotBlank(varid)) {
					asiabpm = asiabpmManager.getAsiabpm(new Integer(varid));
				}
			}
			this.pageContext.setAttribute(this.getVar(), asiabpm);
		} catch (Throwable t) {
			_logger.error("Error in doStartTag", t);
			throw new JspException("Error in AsiabpmTag doStartTag", t);
		}
